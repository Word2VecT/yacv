...
parser_map = {"ll1": LL1Parser, "lr0": LR0Parser, "slr1": SLR1Parser, "lalr1": LALR1Parser, "lr1": LR1Parser}
        ...
        def __init__(self, **kwargs):
            ...
            for k, v in kwargs.items():
                ...
                if key in choices and v not in choices[key]:
                    raise ValueError(
                        'Incorrect config value. Attribute {} must be one out of {}, received "{}"'.format(
                            k, choices[key], v
                        )
                    )
...
def main():
    ...
    if args.parsing_algo == "ll1" and args.vis_automaton:
        ...
        exit(1)
    log.info("Using {} parsing algorithm".format(args.parsing_algo.upper()))
    ...
    if args.vis_automaton:
        fname = "{}-state-automaton.png".format(args.parsing_algo)
        G = p.visualize_automaton()
        G.draw(os.path.join(folder, fname), format="png")
    ...
    if args.vis_parsing:
        ...
        manim_config = get_manim_config(string_folder, fname, args.manim_video_quality)
        if manimce:
            ...
            kwargs = manim_config
        vis = LL1ParsingVisualizer(**kwargs) if args.parsing_algo == "ll1" else LRParsingVisualizer(**kwargs)
